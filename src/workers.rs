...
pub async fn dir_scanner(pool: Pool<SqliteConnectionManager>) -> anyhow::Result<()> {
    loop {
        ...
        for entry in std::fs::read_dir(path).unwrap() {
            ...
            let is_symlink = entry.metadata().map(|md| md.is_symlink()).unwrap_or_else(|_| false);
            if is_symlink {
                continue;
            }
            if p.is_dir() {
...
fn is_text_file(path: &str) -> bool {
    let text_extensions = [
        "txt", "rs", "js", "ts", "py", "java", "c", "cpp", "h", "hpp", "html", "css",
        "md", "json", "yaml", "yml", "toml", "xml", "sh", "bash", "zsh", "fish",
        "sql", "go", "rb", "php", "pl", "pm", "lua", "r", "scala", "kt", "swift",
        "dart", "hs", "ml", "fs", "fsx", "vb", "cs", "fs", "elm", "clj", "cljs",
        "ex", "exs", "nim", "cr", "v", "zig", "jl", "scm", "ss", "rkt", "hy",
        "coffee", "litcoffee", "ls", "moon", "iced", "pl", "pm", "t", "pod",
        "awk", "sed", "makefile", "dockerfile", "gitignore", "readme", "license",
        "changelog", "authors", "contributors", "news", "history", "todo",
    ];
